...
const JobCard = ({ job }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition duration-300 border border-gray-200 relative">
      {/* Card Header */}
      <div className="flex items-center gap-4 mb-4">
        <Avatar company={job.company} />
        <div>
          <h3 className="text-xl font-semibold text-[#3d5a6c]">{job.title}</h3>
          <p className="text-gray-600">{job.company && job.company.name ? job.company.name : "Unknown Company"}</p>
        </div>
      </div>
      ...
      <>
        {job.description && (
          <p className="text-gray-600 text-sm mb-4">{job.description.split(".")[0]}</p>
        )}
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
              saved
                ...
                : "bg-gray-400 text-white hover:bg-[#3d5a6c]"
            }`}
          >
            {saved ? "Saved" : "Save for later"}
          </button>
          <Link to={`/job/${job._id}`} className="bg-[#cbef43] text-[#433a3f] px-4 py-2 rounded-full text-sm font-medium hover:bg-[#8de969] transition-colors">
            Details
          </Link>
        </div>
      </div>
